...
int praseStrToData(unsigned char *str,int length)
{
	...
	if(UartDataCheck(str,length) == 1)
		{	
			...
			source[3].data = (float)((str[31]<<8)+str[32]);	//ÓÐÎÞ¹ÊÕÏ
			source[2].data = (float)((str[29]<<24)+(str[30]<<16)+(str[27]<<8)+str[28]); //Éú²úÊýÁ¿
			...
			if(source[3].data == 1)
				{
					memcpy(source[4].string_data,faulttime_1,14); 	//¹ÊÕÏ·¢ÉúÊ±¼ä
				}
			else if(source[3].data == 0)
				{
					strncpy(source[4].string_data,Nofault,14); 	//¹ÊÕÏ·¢ÉúÊ±¼ä
				}
