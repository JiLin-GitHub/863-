...
#define _MODBUS_RTU_PRESET_REQ_LENGTH  4
#define SLAVE 1
...
static int _modbus_rtu_build_request_basis( int slave, int function, int nb,unsigned char *req )
{
    req[0] = slave;
    req[1] = function;
    //req[2] = addr >> 8;
...
//	printf("opcua adapter start, reviece port %d\n",R_PORT);
	...
	while(1){
				...
				memset(buf,0,20);
				length=_modbus_rtu_build_request_basis(SLAVE,4,10,buf);
