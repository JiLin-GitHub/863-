...
int uart_data_check(unsigned char *str_input)
{
	int i;
	...
	if(str_input == NULL || strlen(str_input) > 55)
	{
...
int praseStrToData(unsigned char *str)
{
	int len=0,i;
	unsigned char *p;
	if(str == NULL) {
	...
	if(uart_data_check(str) == 1) {
		...
		if(p != NULL) {
			...
			memcpy(source[4].string_data+1,p+28,8);
			memcpy(source[5].string_data+1,p+37,7);
			memcpy(source[6].string_data,p+44,1);
			memcpy(source[7].string_data,p+45,1);
...
void set_speed_and_parity(int fd)
{
	...
	cfsetispeed(&Opt, B115200);		//ÉèÖÃ´®¿ÚÊäÈë²¨ÌØÂÊ
	cfsetospeed(&Opt, B115200);		//ÉèÖÃ´®¿ÚÊä³ö²¨ÌØÂÊ
...
//	printf("opcua adapter start, reviece port %d\n",R_PORT);
	...
	while(1){
		...
		if(Serial_fd != 0){
			...
			if(ret == -1){
				perror("select");
			}else if(ret > 0 && FD_ISSET(Serial_fd, &readfd)){ //¼ì²é´®¿Ú¾ä±úÊÇ·ñ»¹´æÔÚ£¿
				//printf("ret = %d\n",ret);
				p = (unsigned char *)malloc(sizeof(unsigned char) * 1000);
				memset(p,0,sizeof(unsigned char)*1000);//³õÊ¼»¯
