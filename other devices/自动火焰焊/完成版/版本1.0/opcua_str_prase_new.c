...
DATA_SOURCE source[]={
		...
		{"down time",1,0.0,{0}},	
		{"fault time",1,0.0,{0}},
		{"cooling time",2,0.0,{0}},
		{"autowork time",2,0.0,{0}},
		{"fire quit standby time",2,0.0,{0}},
...
int praseStrToData(unsigned char *str,int length)
{
	...
	if(UartDataCheck(str,length) == 1)
		{	
			...
			source[3].data = (float)((str[39]<<8)+str[38]);	//ÀäÈ´Ê±¼ä
			source[4].data = (float)((str[41]<<8)+str[40]);	//×Ô¶¯×÷ÒµÊ±¼ä
			source[5].data = (float)((str[43]<<8)+str[42]);	//»ðÑæÏû»ð´ý»úÊ±¼ä
			source[6].data = (float)((str[47]<<24)+(str[46]<<16)+(str[45]<<8)+str[44]); //Éú²úÊýÁ¿
			source[7].data = (float)((str[49]<<8)+str[48]);	//ÓÐÎÞ¹ÊÕÏ
