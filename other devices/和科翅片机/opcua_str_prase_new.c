...
int praseStrToData(unsigned char *str,int length)
{
	...
	if(UartDataCheck(str,length) == 1)
		{	
			...
			source[2].data = (float)((str[29]<<24)+(str[30]<<16)+(str[27]<<8)+str[28]);	//ÓÐÎÞ¹ÊÕÏ
			source[4].data = (float)((str[45]<<24)+(str[46]<<16)+(str[43]<<8)+str[44]); //Éú²ú¼ÆÊýÆ÷
			source[5].data = (float)((str[49]<<24)+(str[50]<<16)+(str[47]<<8)+str[48]);	//³ÉÐÎËÙ¶È
			source[6].data = (float)((str[51]<<8)+str[52]); //É½Êý
			source[7].data = (float)((str[53]<<8)+str[54]); //Õ¹¿ª³¤¶È
			source[8].data = (float)((str[57]<<24)+(str[58]<<16)+(str[55]<<8)+str[56]);	//ÅÄÆ½ËÙ¶È
			source[9].data = (float)((str[61]<<24)+(str[62]<<16)+(str[59]<<8)+str[60]);	//ÇÐ¶ÏËÙ¶È
			source[10].data = (float)((str[65]<<24)+(str[66]<<16)+(str[63]<<8)+str[64]);	//ÔúÀ¥ÊÙÃü
			boottime_1[14] = ' ';
			downtime_1[14] = ' ';
			faulttime_1[14] = ' ';
			NewData[0] = '?';						
			...
			if(source[2].data == 1)
				{
					memcpy(source[3].string_data,faulttime_1,14); 	//¹ÊÕÏ·¢ÉúÊ±¼ä
				}
			else if(source[2].data == 0)
				{
					strncpy(source[3].string_data,Nofault,14); 	//¹ÊÕÏ·¢ÉúÊ±¼ä
				}
