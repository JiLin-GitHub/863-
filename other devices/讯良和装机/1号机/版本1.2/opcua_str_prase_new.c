...
int praseStrToData(unsigned char *str,int length)  //½âÎö
{
		int ret_send = 0;
		int datalen = 0;
		int tmp = 0;
...
void creat_server_sockfd4(int *sockfd, struct sockaddr_in *local, int portnum){
	...
	*sockfd = socket(AF_INET, SOCK_STREAM, 0);
	...
	/*ÉèÖÃÌ×½Ó×ÖÊôÐÔ£¬Ê¹ÆäÔÚ¼ÆËã»úÖØÐÂÆô¶¯Ê±¿ÉÒÔÔÙ´ÎÊ¹ÓÃÌ×½Ó×ÖµÄ¶Ë¿ÚºÍIP
	  ÉèÖÃÌ×½Ó×ÖÊôÐÔ£¬ÓÐ¹ØTCPÐ­ÒéµÄÎÞÑÓÊ±ÊôÐÔ
	...
	*/
	...
	/*
	localÈ«²¿ÇåÁã
	ÉèÖÃÖ¸ÕëlocalµÄµØÖ·×åÀàÐÍ¡ª¡ªIP4
	ÉèÖÃÖ¸ÕëlocalµÄIPºÍportºÅ£¬²¢×ª»¯ÎªÍøÂç×Ö½ÚÐò
	*/
	...
	//½«socketºÍ±¾»úÉÏµÄÒ»¸ö¶Ë¿Ú°ó¶¨£¬Ëæºó¿ÉÔÚ¸Ã¶Ë¿Ú¼àÌý·þÎñÇëÇó
	...
	//socket¼àÌý·þÎñÇëÇó£¬²¢Îª¸Ãsocket½¨Á¢ÊäÈëÊý¾Ý¶ÓÁÐ£¬ÓÃÀ´´æ´¢µ½´ï·þÎñÆ÷µÄÇëÇó£¬µÈ´ý´¦Àí
...
void creatserver(struct argument *p){
	...
	/*µ÷ÓÃacceptº¯Êýºó£¬»á½øÈë×èÈû×´Ì¬  
  accept·µ»ØÒ»¸öÌ×½Ó×ÖµÄÎÄ¼þÃèÊö·û£¬ÕâÑù·þÎñÆ÷¶Ë±ãÓÐÁ½¸öÌ×½Ó×ÖµÄÎÄ¼þÃèÊö·û£¬  
	serverfdºÍp->fd  
	serverfdÈÔÈ»¼ÌÐøÔÚ¼àÌý×´Ì¬£¬p->fdÔò¸ºÔð½ÓÊÕºÍ·¢ËÍÊý¾Ý  
	fromÊÇÒ»¸ö´«³ö²ÎÊý£¬accept·µ»ØÊ±£¬´«³ö¿Í»§¶ËµÄµØÖ·ºÍ¶Ë¿ÚºÅ(±ØÐëÇ¿ÖÆ×ª»»ÎªsockaddrÀàÐÍ)  
	lenÊÇÒ»¸ö´«Èë-´«³ö²ÎÊý£¬´«ÈëµÄÊÇµ÷ÓÃÕßÌá¹©µÄ»º³åÇøµÄfromµÄ³¤¶È£¬ÒÔ±ÜÃâ»º³åÇøÒç³ö¡£  
	´«³öµÄÊÇ¿Í»§¶ËµØÖ·½á¹¹ÌåµÄÊµ¼Ê³¤¶È¡£  
 	³ö´í·µ»Ø-1  ret
 	
 	inet_ntoa   ipµØÖ·×ª»»º¯Êý£¬½«ÍøÂç×Ö½ÚÐòIP×ª»»Îªµã·ÖÊ®½øÖÆIP
  */
	while(1)
	{
...
{ 
	...
	int len=sizeof(info); 
	getsockopt(sock, IPPROTO_TCP, TCP_INFO, &info, (socklen_t *)&len); 	//IPPROTO_TCPÓÃÀ´Ö¸¶¨socket´«ÊäËùÊ¹ÓÃµÄ´«ÊäÐ­Òé-TCP
