...
//			source[2].data = (float)((str[29]<<24)+(str[30]<<16)+(str[27]<<8)+str[28]); //生产数量
	   ...
	   if(SocketConnected(sendOriginalDataFD.fd)) {
					...
					memcpy(NewData_1,NewData,datalen);
					NewData_1[datalen] = '\n';
						ret_send = send(sendOriginalDataFD.fd,NewData_1,datalen+1, 0);						
						if(ret_send < 0) {
							perror("send");
							...
							printf("via wifi to write sendOriginalDataFD_fd success! ret_send=%d\n",ret_send);
							printf("The New String to wifi is:%s\n",NewData);
					}
					else if (SocketConnected(sendOriginalDataFD.fd) == 0)
					{
						printf("wifi socket连接断开,无法上传\n");
					}
