...
int cont_num = 0;	//ÐÂ¼Ó±êÖ¾Î»

int datalen_t = 0;	//TCP·¢ËÍ×Ö½Ú¸öÊý
...
void* soureDataPrase(void *arg)
{
	...
	int i;
  unsigned char *p;
	sleep(1);		//´Ë´¦ÑÓ³Ù1ÃëµÄÄ¿µÄÊÇÎªÁËÊ¹ÁíÒ»¸ö´ò¿ª´®¿ÚµÄ½ø³ÌÖ´ÐÐÍê±Ï
//	printf("opcua adapter start, reviece port %d\n",R_PORT);
	printf("opcua adapter serial data start!!!\n");
	while(1){
					p = (unsigned char *)malloc(sizeof(unsigned char) * 100);
